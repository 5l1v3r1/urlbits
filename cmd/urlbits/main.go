...
func init() {
	...
	flag.Usage = func() {
		...
		h := "USAGE:\n"
		h += "  urlbits [OPTIONS] [MODE] [FORMATSTRING]\n\n"
		...
		h += "EXAMPLES:\n"
		h += "  cat urls.txt | urlbits keys\n"
		h += "  cat urls.txt | urlbits format %s://%h%p?%q\n"
...
type urlProc func(*gos.URL, string) []string
...
func keys(u *gos.URL, _ string) []string {
	out := make([]string, 0)
...
func values(u *gos.URL, _ string) []string {
	out := make([]string, 0)
...
func keyPairs(u *gos.URL, _ string) []string {
	out := make([]string, 0)
...
func hostnames(u *gos.URL, f string) []string {
	return format(u, "%h")
}
...
// for http://sub.example.com/path it will return
// []string{"/path"}
func paths(u *gos.URL, f string) []string {
	return format(u, "%p")
}
...
func format(u *gos.URL, f string) []string {
	out := &bytes.Buffer{}
	...
	for _, r := range f {
		...
		case 'S':
			out.WriteString(u.SubDomainName)

		// the root; e.g. example
		case 'r':
			out.WriteString(u.BaseDomainName)
...
func main() {
	...
	for scanner.Scan() {
		parsedURL, err := gos.ParseURL(scanner.Text())
		if err != nil {
			if co.verbose {
